...
public interface CompensationClaimRepository extends JpaRepository<CompensationClaim, Integer> {
    ...
    @Query("Select c FROM CompensationClaim c " +
            ...
            "AND c.compensationClaimStatus = 'APPLIED'" +
            "AND c.compensationClaimStatus = 'UPDATED'" +
            "AND c.compensationClaimStatus = 'APPROVED'")
    List<CompensationClaim> findAppliedUpdatedApprovedByClaimingEmployeeId(@Param("employee_id") int employee_id);
}
